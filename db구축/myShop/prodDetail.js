// ====================================================================
// 상품 상세정보 전담 모듈 (prodDetail.js) - MySQL 버전
// ====================================================================
// 
...
// 
// 주요 처리 흐름:
// 1. 상품 기본정보 조회 (Products 테이블)
// 2. 상품평 정보 조회 (Prod_evals + Customers JOIN)
 ...
 * 
 * 기능: Products 테이블에서 특정 상품의 모든 기본 정보를 조회
 * 반환 필드: prod_cd, prod_name, price, prod_type, material, prod_img, prod_intro
 * 
 * 사용 예시:
...
async function getProductInfo(prod_cd) {
  const [productInfo] = await db.query(
    `SELECT prod_cd, prod_name, price, prod_type, material, prod_img, prod_intro
     FROM Products 
     WHERE prod_cd = ?`,
    [prod_cd]                                           // SQL 인젝션 방지를 위한 파라미터화된 쿼리
  );
  return productInfo[0] || null;                        // 첫 번째 결과 반환, 없으면 null
}
 ...
 * JOIN 구조: Prod_evals + Customers (cust_id 기준)
 * 정렬: eval_seq_no DESC (최신 평가부터 표시)
 * 
 * 반환 구조:
 * [
 *   {
...
async function getProductReviews(prod_cd) {
  const [reviews] = await db.query(
    `SELECT pe.eval_score, pe.eval_comment
          , rpad(substring(cu.cust_name,1,1),char_length(cu.cust_name),'*') cust_name -- 고객 성명은 왼쪽 한 글자만 노출
          , pe.eval_seq_no
     FROM Prod_evals pe
     JOIN Customers cu ON pe.cust_id = cu.cust_id              
     WHERE pe.prod_cd = ?
     ORDER BY pe.eval_seq_no DESC`,                     // 최신 평가부터 정렬
    [prod_cd]
  );
            ...
            <div class="main-content">
                    ...
                    <div class="product-info">
                        ...
                        <div class="product-details">
                            ...
                            <div class="detail-row">
                                ...
                                <span class="detail-value">${product.prod_cd}</span>
                            </div>
                            <div class="detail-row">
                ...
                <div class="product-description">
                    ...
                    <div class="description-content">
                        ${product.prod_intro || '상품 소개가 준비되어 있지 않습니다.'}
                        <!-- prod_intro가 null/undefined인 경우 기본 메시지 표시 -->
                    </div>
                </div>
