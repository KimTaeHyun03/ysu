/**
 * ========================================
 * myPage.js - 마이페이지 관리 모듈 (MongoDB 버전)
 * ========================================
 * 
 * 설명: 사용자의 개인정보, 주문내역, 상품평가 관리를 담당하는 Express 라우터 모듈
 * 주요 기능:
 * - 고객 정보 조회 및 표시
 * - 주문 내역 조회 및 그룹화 (Aggregation Pipeline 사용)
 * - 상품 평가 등록/수정/삭제
 * - 서버 사이드 렌더링 및 API 제공
 */
 ...
 * - cust_name: 고객명
 * - m_phone: 휴대폰 번호
 * - agreements.terms: 이용약관 동의 여부 ('Y'/'N')
 * - agreements.privacy: 개인정보 처리방침 동의 여부 ('Y'/'N')
 * - agreements.marketing: 마케팅 수신 동의 여부 ('Y'/'N')
 */
async function getCustInfo(cust_id) {
  ...
  console.log('🎯 조회 결과:', custInfo);
  return custInfo;
}

/**
 * 공통 함수: 주문 내역 조회 (MongoDB Aggregation Pipeline 사용)
 * @param {string} cust_id - 고객 ID
 * @returns {Array} 주문 내역 배열 (상품 정보와 평가 정보 포함)
 * 
 * 설명: MongoDB Aggregation Pipeline을 사용하여 복잡한 JOIN 작업을 수행합니다.
 *      여러 컬렉션(Orders, Products, Reviews)을 연결하여 완전한 주문 정보를 조회합니다.
 * 
 * - Orders 컬렉션의 items 임베드 배열 처리
 * - Products, Reviews 컬렉션과 조인
 * - unit_price 필드 사용으로 정확한 주문 가격 반영
 * 
 * 정렬 순서:
 * 1. 주문일 내림차순 (최신 주문 우선)
...
async function getPurchaseHistory(cust_id) {
  const purchases = await db.collection('Orders').aggregate([
    {
      // 특정 고객의 주문만 필터링
      $match: {
        cust_id: cust_id
    ...
    {
      // 임베드된 items 배열을 개별 문서로 분리
      $unwind: '$items'
    },
    {
      // Products 컬렉션과 조인 (상품 기본 정보)
      $lookup: {
        from: 'Products',
        localField: 'items.prod_cd',
        foreignField: '_id',
        as: 'product_info'
      }
    },
    ...
    {
      // Reviews 컬렉션과 조인 (복합 조건 매칭)
      $lookup: {
        from: 'Reviews',
        let: {
          ordNo: '$ord_no', 
          custId: '$cust_id',
          ordItemNo: '$items.ord_item_no'
        },
        pipeline: [
          {
            $match: {
    ...
    },
    {
      ...
      $project: {
        ...
        // 상품 정보 (Products 컬렉션)
        prod_cd: '$product_info._id',
        prod_name: '$product_info.prod_name',
        price: '$product_info.price',
        prod_img: '$product_info.prod_img',
        prod_type: '$product_info.prod_type',
        material: '$product_info.material',
        
        // 주문 상세 정보 (임베드된 items)
        ord_item_no: '$items.ord_item_no',
        prod_size: '$items.prod_size',
        ord_qty: '$items.ord_qty',
        unit_price: '$items.unit_price',     // 주문 시점의 실제 가격
        review_written: '$items.review_written',
        
        // 리뷰 정보 (선택적)
        eval_seq_no: { 
          $cond: {
            if: { $gt: [{ $size: '$review_info' }, 0] },
            then: { $toString: { $arrayElemAt: ['$review_info._id', 0] } },
            else: null
          }
        },
        eval_score: { $arrayElemAt: ['$review_info.eval_score', 0] },
        eval_comment: { $arrayElemAt: ['$review_info.eval_comment', 0] }
      }
    },
    {
      // 최신 주문부터 정렬
      $sort: {
        ord_date: -1,            // 1순위: 주문일 내림차순 (최신 주문 우선)
        ord_no: -1,              // 2순위: 주문번호 내림차순 (같은 날짜 내에서 최신 주문 우선)
        'items.ord_item_no': 1   // 3순위: 주문아이템번호 오름차순 (주문 내 아이템 정렬)
      }
    }
  ]).toArray();
 ...
 * @param {Array} purchases - 주문 내역 배열 (Aggregation Pipeline 결과)
 * @returns {Array} 주문별로 그룹화된 배열 (계층적 구조)
 * 
 * 설명: Aggregation Pipeline으로 인해 플랫하게 반환된 데이터를 주문별로 그룹화하여
 ...
 * 처리 과정:
 * 1. 사용자 인증 정보 확인 및 추출
 * 2. 고객 정보 조회 (MongoDB findOne)
 * 3. 주문 내역 조회 및 그룹화 (MongoDB Aggregation Pipeline)
...
router.get('/', async (req, res) => {
  try {
    ...
    // 주문 내역 조회 및 그룹화 (MongoDB Aggregation Pipeline)
 ...
 * - 사용자 인증 및 입력값 검증 포함
 * - MongoDB upsert 기능 활용으로 등록/수정 로직 단순화
 * 
 * 요청 형식:
 * {
...
router.post('/rating', async (req, res) => {
  try {
    ...
    // 주문번호도 함께 저장하기 위해 Orders에서 주문번호 조회
    ...
    const result = await db.collection('Reviews').updateOne(
      { 
        cust_id: userid,
        ord_item_no: parseInt(ord_item_no)  // 숫자로 변환하여 매칭
      },
      {
        $set: {
          ord_no: orderInfo.ord_no,           // 주문번호 저장
          prod_cd: prod_cd,
          cust_id: userid,
          ord_item_no: parseInt(ord_item_no), // 숫자로 저장
    ...
    // Orders 컬렉션의 review_written 플래그도 업데이트
    ...
    if (result.upsertedCount > 0) {
      ...
      console.log('ℹ️ 평가 데이터 변경 없음:', { userid, ord_item_no, eval_score });
 ...
 * 
 * MongoDB 설계 원칙:
 * - ObjectId 또는 커스텀 ID를 사용한 문서 식별
 * - deleteOne() 메서드로 안전한 단일 문서 삭제
 * - 삭제 결과 객체로 성공/실패 여부 확인
 * 
 * URL 형식: DELETE /mypage/rating/:eval_seq_no
 * 파라미터:
 * - eval_seq_no: 삭제할 평가의 고유 번호 (MongoDB에서는 _id 또는 커스텀 필드)
 * 
 * 응답 형식:
 * {
 *   message: string,      // 성공/실패 메시지
...
router.delete('/rating/:eval_seq_no', async (req, res) => {
  try {
    ...
    console.log('🗑️ 평가 삭제 요청:', { userid, eval_seq_no });
    ...
    if (result.deletedCount > 0) {
      ...
      console.log('⚠️ 삭제할 평가 없음:', { userid, eval_seq_no });
 ...
 * 데이터 처리 과정 (MongoDB):
 * 1. 사용자 인증 확인
 * 2. Aggregation Pipeline으로 주문내역 조회
 * 3. JavaScript로 주문별 그룹화 처리
 * 4. 통계 정보 계산
 * 5. JSON 응답 생성
 */
router.get('/api/orders', async (req, res) => {
  try {
    ...
    // 주문내역 조회 (MongoDB Aggregation Pipeline 사용)
    const purchases = await getPurchaseHistory(userid);
    const orderGroups = groupPurchasesByOrder(purchases);
 ...
 * - POST /mypage/rating        : 상품 평가 등록/수정 (MongoDB upsert 사용)
 * - DELETE /mypage/rating/:id  : 상품 평가 삭제 (ObjectId 또는 커스텀 ID 지원)
 * - GET  /mypage/api/orders    : 주문내역 JSON API (Aggregation Pipeline)
 * - GET  /mypage/api/profile   : 고객정보 JSON API (findOne)
 * 
 * MongoDB 버전의 라우터 특징:
 * - NoSQL 스키마리스 구조의 유연성 활용
 * - Aggregation Pipeline을 통한 복잡한 데이터 조회
 * - upsert 기능으로 등록/수정 로직 단순화
 * - ObjectId와 커스텀 ID 혼용 지원
 * - 일관된 오류 처리 및 로깅 체계 유지
