/**
 * ====================================================================
 * 의류쇼핑몰 메인 서버 (Main.js) - MongoDB 버전
 * ====================================================================
 * 
...
const db = require('./db.js');                              // MongoDB 연결 모듈 - MongoDB 연결 관리
  ...
  <body>
    ...
    <script>
      ...
      function createProductCardHTML(product) {
        ...
        var prodCd = product.prod_cd || product._id || 'unknown';
 ...
 * ================================================================
 * 
 * 기능: MongoDB에서 모든 상품 정보를 조회하여 JSON 형태로 반환
 * 
 * 응답 형식:
 * [
 *   {
 ...
 * ]
 * 
 * 에러 처리: MongoDB 연결 오류, 쿼리 실행 오류 등
 * ================================================================
 */
app.get('/api/products', async (req, res) => {
  ...
  try {
    // MongoDB에서 모든 상품 정보를 조회
    const products = await db.collection('Products').find({}).toArray();
    
    console.log('📊 상품 조회 성공, 개수:', products.length);
 ...
 * 1. 요청 본문에서 상품 정보 추출
 * 2. 세션에서 사용자 정보 확인
 * 3. MongoDB customers 컬렉션의 사용자 장바구니 배열에 추가
 * 4. 성공/실패 응답 전송
 * 
 * 보안: 로그인된 사용자만 접근 가능
 * ================================================================
...
app.post('/cart/add', async (req, res) => {
  ...
  try {
    ...
    // 장바구니 아이템 객체 생성
    const cartItem = {
      cart_seq_no: new Date().getTime(), // 유니크한 시퀀스 번호 (timestamp 사용)
      prod_cd: pcd,
      prod_size: size,
      ord_qty: parseInt(qty),
      ord_yn: "N",
      added_date: new Date()
    };
    
    console.log('🛒 추가할 장바구니 아이템:', cartItem);
    
    // MongoDB에서 사용자의 장바구니 배열에 새 아이템 추가
    // $push 연산자를 사용하여 cart 배열에 새 아이템 추가
    const updateResult = await db.collection('Customers').updateOne(
      { _id: userid },                    // 사용자 ID로 문서 찾기
    ...
    res.json({ 
      ...
      success: true,
      cartItemId: cartItem.cart_seq_no
    });
    
    console.log('📤 장바구니 추가 성공 응답 전송 완료');
    
  } catch (error) {
    // MongoDB 오류 또는 기타 서버 오류 처리
 ...
 * 
 * 1. 상품 조회 API 변경:
 *    MySQL: const [results] = await db.query('SELECT * FROM Products');
 *    MongoDB: const products = await db.collection('Products').find({}).toArray();
 * 
 * 2. 장바구니 추가 방식 변경:
 *    MySQL: INSERT INTO Carts(...) VALUES(...)
 *    MongoDB: db.collection('Customers').updateOne({_id: userid}, {$push: {cart: cartItem}})
 * 
 * 3. 데이터 구조 개선:
 *    - 장바구니를 별도 컬렉션에서 사용자 문서 내 배열로 임베딩
 *    - cart_seq_no를 timestamp 기반으로 생성
 *    - added_date 필드 자동 추가
 * 
 * 4. 에러 처리 개선:
 *    - MongoDB 특화 에러 처리
 *    - matchedCount, modifiedCount 확인
 * 
 * ====================================================================
 * 모듈 시스템 정보 (참고용) - MongoDB 버전
 * ====================================================================
 * 
 * 이 애플리케이션의 모듈 의존성:
 * 
 ...
 * 
 * MongoDB 컬렉션:
 * - customers: 고객 정보 + 임베딩된 장바구니
 * - products: 상품 정보  
 * - orders: 주문 정보 + 임베딩된 주문 상세
 * - reviews: 상품 리뷰
 * 
 * ====================================================================
 * 보안 고려사항 (MongoDB 버전)
